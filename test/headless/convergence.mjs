// Two clients sync a Yjs document through the yrb-lite server using the
// YrbLiteProvider: edits propagate both ways and a late joiner is brought up to
// date by the server.
...
function client() {
  ...
  const provider = new YrbLiteProvider(doc, rawConsumer(URL), "DocumentChannel", { id: ROOM }, { awareness });
  provider.connect();
