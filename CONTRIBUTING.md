...
[`yrb-lite`](https://github.com/jpcamara/yrb-lite) (the Rust-backed Yjs server
+ `yrb-lite-client`) is the other half — so most of the test suite drives a real
`yrb-lite` Rails server.

## Prerequisites

- [Bun](https://bun.sh) (build + test runner)
- A local checkout of [`yrb-lite`](https://github.com/jpcamara/yrb-lite) **as a
  sibling directory** (`../yrb-lite`). The test server's `Gemfile` path-pins the
  `yrb-lite` and `yrb-lite-actioncable` gems to that checkout.
- For the server-backed tests: Ruby 3.4+ and a Rust toolchain (the `yrb-lite`
...
`test/run.mjs` boots the `yrb-lite` test server (`test/server`, a minimal Rails
