 ...
 */
export class LexxyCursorManager {
  constructor(editor, provider, containerElement) {
    ...
    // State management
    this.cursors = new Map(); // clientId -> cursor data
    this.selections = new Map(); // clientId -> selection data
    this.localClientId = this.awareness.clientID;
    this.lastSeenTimestamps = new Map(); // clientId -> last seen timestamp
    ...
    // Throttling
    this.lastUpdate = 0;
    this.UPDATE_THROTTLE = 16; // ~60fps

    // Inactive user timeout (30 seconds)
    this.INACTIVE_TIMEOUT = 30000;
  ...
  init() {
    ...
    // Listen to DOM mutations for dynamic content
    ...
    // Initial render of existing awareness states (with delay to ensure provider is connected)
    setTimeout(() => {
  ...
  sendHeartbeat() {
    // Update our own timestamp in awareness
  ...
  checkInactiveUsers() {
    ...
    states.forEach((state, clientId) => {
      if (clientId === this.localClientId) return; // Skip local user

      const lastSeen = state?.lastSeen;
      if (lastSeen) {
        const timeSinceLastSeen = now - lastSeen;
        if (timeSinceLastSeen > this.INACTIVE_TIMEOUT) {
          console.log(`Removing inactive user ${clientId} (last seen ${Math.round(timeSinceLastSeen / 1000)}s ago)`);
          this.removeCursor(clientId);
          this.removeSelection(clientId);
          this.lastSeenTimestamps.delete(clientId);
        } else {
          this.lastSeenTimestamps.set(clientId, lastSeen);
        }
      }
    });
  }
      ...
      [...added, ...updated].forEach(clientId => {
        ...
        try {
          ...
          if (state) {
            ...
            if (state.lastSeen) {
              this.lastSeenTimestamps.set(clientId, state.lastSeen);
            }
  ...
  handleEditorUpdate({ editorState }) {
    // Update positions after editor changes
    editorState.read(() => {
  ...
  updateCursorFromState(clientId, state) {
    ...
    // Validate cursor has the required structure
    ...
    const cursorData = {
      clientId,
      user: user || { name: `User ${clientId}`, color: this.getColorForClient(clientId) },
    ...
    // Check if it's a selection or just a caret
    if (this.isCollapsed(cursor.anchorPos, cursor.focusPos)) {
      this.removeSelection(clientId);
      this.selections.delete(clientId);
    } else {
      this.selections.set(clientId, cursorData);
    }
  }

  isCollapsed(anchor, focus) {
    // Safely check for collapsed state
  ...
  updateSelectionPosition(clientId, selectionData) {
    ...
    // Create rect elements for each selection rect
    rects.forEach(rect => {
      const rectElement = document.createElement('div');
      rectElement.className = 'lexxy-selection-rect';
      // Use a more visible color with proper opacity
      ...
      rectElement.style.cssText = `
        position: absolute;
        background-color: ${hexToRgba(user.color, 0.3)};
        left: ${rect.x}px;
        top: ${rect.y}px;
        width: ${rect.width}px;
  ...
  getPositionFromPoint(point) {
    ...
    try {
      // Special handling for empty nodes (like new paragraphs)
      ...
      // Fallback to domNode position if range rect is invalid
  ...
  createRangeFromPoint(domNode, offset) {
    ...
    if (domNode.nodeType === Node.TEXT_NODE) {
      ...
      if (targetNode) {
        ...
        // Fallback to start/end of element without modifying DOM
        try {
          range.selectNodeContents(domNode);
          range.collapse(offset === 0);
        } catch (e) {
          // If that fails, just use the node itself
  ...
  getSelectionRects(anchor, focus) {
    // We need to read this within the editor state
    return this.editor.getEditorState().read(() => {
      ...
      try {
        ...
        // Determine which comes first in document order
        const isBackward = anchor.key === focus.key ?
          anchor.offset > focus.offset :
          anchorDOM.compareDocumentPosition(focusDOM) & Node.DOCUMENT_POSITION_PRECEDING;
  ...
  getColorForClient(clientId) {
    // Large palette of distinct colors to minimize overlap
    const colors = [
      // Reds & Pinks
      '#FF6B6B', '#FF006E', '#C1121F', '#FF4365', '#E63946',
      '#DC2F02', '#D00000', '#9D0208', '#FF0A54', '#FF5C8A',

      // Oranges & Yellows
      '#F77F00', '#FCBF49', '#FFB700', '#FFA400', '#FF9500',
      '#F4A261', '#E76F51', '#FF8C42', '#FFD60A', '#FFB627',

      // Greens
      '#06FFA5', '#00F5FF', '#55A630', '#2D6A4F', '#52B788',
      '#40916C', '#1B5E20', '#2E7D32', '#43A047', '#66BB6A',
      '#4ECDC4', '#96CEB4', '#88D498', '#36C9C6', '#06FFB4',

      // Blues
      '#0077B6', '#0096C7', '#00B4D8', '#48CAE4', '#90E0EF',
      '#023E8A', '#0466C8', '#5390D9', '#4361EE', '#4895EF',
      '#4CC9F0', '#45B7D1', '#2196F3', '#1976D2', '#0D47A1',

      // Purples & Violets
      '#7209B7', '#B5179E', '#F72585', '#7B2CBF', '#6A4C93',
      '#6C5CE7', '#A8E6CF', '#DDA0DD', '#9C27B0', '#8E24AA',
      '#AB47BC', '#CE93D8', '#6A1B9A', '#4A148C', '#9D4EDD',

      // Browns & Earth tones
      '#8D5524', '#A0522D', '#964B00', '#6F4E37', '#8B4513',

      // Teals & Cyans
      '#14213D', '#006D77', '#2C7DA0', '#468FAF', '#61A5C2',
      '#007EA7', '#003459', '#00A8CC', '#005F73', '#0A9396',

      // Unique colors
      '#540B0E', '#702632', '#A4133C', '#800E13', '#590D22',
      '#370617', '#6A040F', '#9D0208', '#D00000', '#DC2F02'
    ];

    // Use both clientId and a hash for better distribution
    let hash = 0;
 ...
 */
export function syncLocalCursorPosition(editor, provider) {
  ...
  return editor.registerUpdateListener(() => {
    editor.getEditorState().read(() => {
      ...
      // Use the standard Yjs cursor format with anchorPos and focusPos
      const cursorState = {
        anchorPos: {
          key: anchor.key,
      ...
      // Set cursor field following Yjs editor binding convention
